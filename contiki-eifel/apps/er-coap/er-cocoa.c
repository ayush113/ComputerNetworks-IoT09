		  ...
		  for (t = (coap_rtt_estimations_t*)list_head(rtt_estimations_list); t; t = t->next){
		    if (uip_ipaddr_cmp(transactionAddr, &t->addr)){
		    	...
		    	if(retransmissions == 0){
		    		...
		    		t->rttsmissed = 0;
				double delta = rtt - t->rtt[rttType];
				double gain = 1/3;
				double gainBar = delta - t->rttvar[rttType];
				...
				if(t->rto[rttType] == COAP_INITIAL_RTO){
					...
					t->rtt[rttType] = t->rtt[rttType] + gain * delta;

					if (gainBar>=0){

						if(delta>=0)
							t->rttvar[rttType] = t->rttvar[rttType] + gain * gainBar;
						else
							t->rttvar[rttType] = t->rttvar;		
					}
					else{
						
						if(delta>=0)
							t->rttvar[rttType] = t->rttvar[rttType] + (gain*gain) * gainBar;
						else
							t->rttvar[rttType] = t->rttvar;	
						
					}
				}
			t->rto[rttType] = (t->rtt[rttType] + 1/gain * t->rttvar[rttType]> rtt + G)?t->rtt[rttType] + 1/gain * t->rttvar[rttType]:rtt+G;
